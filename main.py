...
from core.groq import tao_tu_khoa_google
...
def chatbot_response(user_input, user_context):
    try:
        ...
        ket_qua = tao_tu_khoa_google(user_input, user_context["history"])
        if not ket_qua["success"]:
            xoa_ngucanh(user_context)
            user_context["history"] = user_context["history"][-2:]
